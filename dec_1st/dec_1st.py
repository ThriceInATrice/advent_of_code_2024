...
# in the right list. Calculate a total similarity score by adding up each number in the left
...
def get_sum_of_differenes(filepath):
    """this sorts each column and finds the differences between the nth member of each list,
    then sums those differences"""
    # get data from file
    with open(filepath) as file:
        file_data = [line.split("   ") for line in file]
        left_column = [int(entry[0]) for entry in file_data]
        right_column = [int(entry[1]) for entry in file_data]
    ...
    # sum differences
    difference_sum = sum(
        [abs(left_column[i] - right_column[i]) for i in range(len(left_column))]
    )

    return difference_sum


def get_similarity_sum(filepath):
    """
    this function solves the second part of todays problem by using a list comprehension
    to add up each instance in the right column of each instance in the left column
    """
    # get data from file
    with open(filepath) as file:
        file_data = [line.split("   ") for line in file]
    ...
    similarity_sum = sum(
        [
            sum([item for item in right_column if item == element])
...
if __name__ == "__main__":
    print(f"SUM OF DIFFERENCES: {get_sum_of_differenes('dec_1st/input_1.txt')}")
    print(f"SUM OF SIMILARITY: {get_similarity_sum('dec_1st/input_1.txt')}")
