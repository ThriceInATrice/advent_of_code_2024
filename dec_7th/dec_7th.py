...
def sum_possible_calibrations(filepath):
    ...
    return sum([calibration_check(calibration) for calibration in calibrations])


def calibration_check(calibration):
    target, nums = calibration
    if len(nums) == 2:
        return target if (nums[0]*nums[1] == target or nums[0]+nums[1] == target) else False
    
    else:
        if (nums[0]*nums[1] == target or nums[0]+nums[1] == target):
            return target

        if nums[0]*nums[1] < target:
            next_step = calibration_check([target, [nums[0]*nums[1]]+nums[2:]])
            if next_step:
                return next_step
...
if __name__ == "__main__":
    print(
        f"SUM OF POSSIBLE CALIBRATIONS: {sum_possible_calibrations('dec_7th/input_1.txt')}"
    )
