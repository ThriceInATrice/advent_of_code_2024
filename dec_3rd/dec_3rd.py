...
def get_muls(file_path):
    ...
    with open(file_path) as file:
        return sum(
            do_mul(mul_string)
            for mul_string in re.findall("mul\([0-9]{1,3},[0-9]{1,3}\)", file.read())
        )
...
def get_do_dont_muls(file_path):
    ...
    resulting strings around 'don't()' and takes the first element (ie the part of the string
    after 'do()' but before 'dont()'). then it runs the same regex as before and calls do_mul
    to process the relevant strings"""
    with open(file_path) as file:
        return sum(
            [
                sum(
                    do_mul(mul_string)
                    for do_dont_string in file.read().split("do()")
                    for mul_string in re.findall(
                        "mul\([0-9]{1,3},[0-9]{1,3}\)",
                        do_dont_string.split("don't()")[0],
                    )
                )
            ]
        )
...
if __name__ == "__main__":
    print(f"MULS: {get_muls('dec_3rd/input_1.txt')}")
    print(f"DO DONT MULS: {get_do_dont_muls('dec_3rd/input_1.txt')}")
